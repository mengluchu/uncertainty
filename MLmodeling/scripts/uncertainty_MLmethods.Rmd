...
```{r, eval=FALSE}
packages <- c("glmnet",  "stargazer",   "CAST",   "quantregForest",   "h2o", "dismo","scoringRules")
...
```{r}
mergedall = read.csv("https://raw.githubusercontent.com/mengluchu/uncertainty/master/data_vis_exp/DENL17_uc.csv") #482
```
...
Remove stations with less than 25% of the records, there are 8 in Germany, non in the Netherlands (one with 25.4% but we included it).  The 8 stations in Germany have missing values 50% and more. Four rural and 4 urban. 
```{r rm, eval = FALSE}

DENL_hr = fread("~/Documents/GitHub/gwr_hr/denl_hourly.csv")
DENL_hr = DENL_hr%>%filter(Countrycode=="DE")
d = pivot_wider(DENL_hr, names_from = AirQualityStation, values_from = value_g)
cna =function(x)
  {x = unlist(x)
  sum(is.na(x))/ length(x)}

nl = sapply(d, cna) #    Min.  1st Qu.   Median     Mean  3rd Qu.     Max.
#0.000000 0.009075 0.019064 0.027385 0.027568 0.253767
 
nl[which(nl > 0.25)]
#STA.DE_DEHE135 STA.DE_DEHH049 STA.DE_DENW381 STA.DE_DENW382 STA.DE_DESH028
#0.5851598      0.9170091      0.7307078      0.7947489      0.8373288
#STA.DE_DESH033 STA.DE_DESH057 STA.DE_DESL018
#0.5325342      0.4848174      0.8373288
save(nl, "~/Documents/GitHub/uncertainty/data_vis_exp/missing_percentage.rda")
mergedall =mergedall%>%filter(AirQualityStation %in% names(which(nl > 0.25)))
...
INLA = INLA_crossvali2(d = orderedall, dp =orderedall, formula = formula, covnames = covnames,   typecrossvali = "non-spatial", family = "gaussian",training=training, test=test)

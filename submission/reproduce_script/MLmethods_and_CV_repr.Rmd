...
XGB
```{r xgb, eval=T}
xgb_crossvali =  function(n,df, y_var,typecrossvali =  "non-spatial" ,coordi = c(mergedall$Longitude, mergedall$Latitude)) {
...
```{r, eval = T}
rf_crossvali =  function(n,df, y_var, coordi = c(mergedall$Longitude, mergedall$Latitude), typecrossvali =  "non-spatial" ) {
  smp_size <- floor(0.2 * nrow(df)) 
  set.seed(n)
  if(typecrossvali == "non-spatial"){
    test <- sample(seq_len(nrow(df)), size = smp_size)
...
LA
```{r, eval=T}
LA_crossvali =  function(n,df, y_var,typecrossvali = "non-spatial" , coordi = c(mergedall$Longitude, mergedall$Latitude)) {
  smp_size <- floor(0.2 * nrow(df)) 
  set.seed(n)
  if(typecrossvali == "non-spatial"){
    test <- sample(seq_len(nrow(df)), size = smp_size)
...
```{r}
q_rf_crossvali =  function(n,df, y_var,typecrossvali =  "non-spatial" , coordi = c(mergedall$Longitude, mergedall$Latitude)) {
  smp_size <- floor(0.2* nrow(df)) 
  set.seed(n)
   if(typecrossvali == "non-spatial"){
    test <- sample(seq_len(nrow(df)), size = smp_size)
...
rf_la_crossvali =  function(n,df, y_var,typecrossvali = "non-spatial", coordi = c(mergedall$Longitude, mergedall$Latitude)) {
  smp_size <- floor(0.2* nrow(df)) 
  set.seed(n)
   if(typecrossvali == "non-spatial"){
    test <- sample(seq_len(nrow(df)), size = smp_size)
...
```{r, eval=F}
df = data.frame(Lasso = VLA, RF = Vrf, XGB = V2 , RF_Lasso = rfla[1:8,])
